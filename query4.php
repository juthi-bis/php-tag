...
<h2><em>Total cost of Guest_id 40</em></h2>
<br>
<?php
...
$result = mysql_query("SELECT Name,Food_cost,Service_cost,SUM(Food_cost+Service_cost) AS Total_cost FROM accomodation_guest_info AS x LEFT JOIN(SELECT b.Guest_id,SUM(Rate*Quantity) AS Food_cost FROM food AS a LEFT JOIN food_order AS b ON a.Product_id=b.Product_id WHERE Guest_id=40) AS y on x.Guest_id=y.Guest_id LEFT JOIN(SELECT d.Guest_id,SUM(Rate*Quantity) AS Service_cost FROM services AS c LEFT JOIN services_order AS d ON c.Sl_no=d.Sl_no WHERE Guest_id=40) AS z on y.Guest_id=z.Guest_id WHERE x.Guest_id=40");
